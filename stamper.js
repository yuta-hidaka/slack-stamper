...
// Comment url that you want add the emoji.(That only channel you granted)
const url = "";
const paths = url.split("/");
const path = paths[paths.length - 1].substring(1);

// YOURE TOKEN
const token = "";

// channel ID
const channel = "C0xxxxx";
const MAX_REACT_NUM = 22;

const main = () => {
  const number = Math.floor(Math.random() * 10) + 1;
  let cnt = 0;
  let i = 0;
  let stampList = ["noushi"];
  for (const v of Object.keys(stamps)) {
    if (cnt > MAX_REACT_NUM) break;
    if (i % number === 0) {
      cnt++;
      stampList.push(String(v));
    }
    i++;
  }
  ...
  for (const stamp of stampList) {
    ...
    var reaction_options = {
      ...
      json: {
        ...
        channel: channel,
        timestamp:
          path.slice(0, path.length - 6) + "." + path.slice(path.length - 6),
      },
    };
...
// src : https://gist.github.com/kjohnson/cf3a03e3baf3ec5762e0442e30c1718a
const stamps = {
  100: "1f4af",
  1234: "1f522",
  interrobang: "2049",
